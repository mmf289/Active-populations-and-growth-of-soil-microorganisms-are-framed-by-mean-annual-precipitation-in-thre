...
data.corr.melted.ang <- read.table("data.corr.melted.ang.txt", header = TRUE, sep = "\t")
...
data.corr.melted.ang<- data.corr.melted.ang %>%
  select(-X)

data.corr.melted.ang$Isotope_Treatment<- paste(data.corr.melted.ang$Isotope_Treatment,"O",sep = "")
data.corr.melted.ang$Fraction<-as.factor(data.corr.melted.ang$Fraction)
data.corr.melted.ang$Tube<-as.factor(data.corr.melted.ang$Tube)
data.corr.melted.ang$Isotope_Treatment<-as.factor(data.corr.melted.ang$Isotope_Treatment)
data.corr.melted.ang$Soil_Isotope<-as.character(data.corr.melted.ang$Soil)
data.corr.melted.ang$Sample_ID<-as.character(data.corr.melted.ang$Sample_ID)
data.corr.melted.ang$taxon<-as.factor(data.corr.melted.ang$taxon)
...
data.melted.heavyfilt <- data.corr.melted.ang %>%
  group_by(taxon, Tube) %>%
...
join.test <- left_join(data.corr.melted.ang, data.melted.heavyfilt)
...
#Sedgwick
data.melted.angelo <- filter.taxa(DATA=data.melted.heavyfilt, trt.code.1= Tcompare$trt.code.1[2], trt.code.2= NULL, trt.refs=NULL, vars=c("taxon", "copies.ul", "Tube", "Soil_Isotope"), min.reps=2)
data.melted.angelo <- filter.taxa(DATA=data.melted.angelo, trt.code.1=NULL, trt.code.2=Tcompare$trt.code.2[2], trt.refs=NULL, vars=c("taxon", "copies.ul", "Tube", "Soil_Isotope"), min.reps=2)

#Files are ready for all.taxa.calcs
set.seed(100)

system.time(all.comparisons.angelo <- all.taxa.calcs(X.all=data.melted.angelo, comparisons=Tcompare[2,], vars=c("taxon", "Density_g_ml", "copies.ul", "Tube", "Soil_Isotope", "DNA_ng_ul"), growth.model="exponential", prop.O.from.water=0.6, v.frac=50, copies.cell=6, pgC.cell=0.1, CI=0.95, draws=1000, tailed.test=1))
write.csv(all.comparisons.angelo, file = "qSIP_output/all.comparisons.angelo.csv")
