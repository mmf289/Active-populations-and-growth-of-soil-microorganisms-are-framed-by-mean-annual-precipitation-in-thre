...
data.corr.melted.sed <- read.table("data.prep.and.calcs/data.corr.melted.sed.txt", header = TRUE, sep = "\t")
Tcompare <- read.table("data.prep.and.calcs/trt.comparisons.txt", header=TRUE, sep="\t", colClasses=c("numeric","factor","character","character","character","numeric","character"))
...
data.corr.melted.sed<- data.corr.melted.sed %>%
  select(-X)

data.corr.melted.sed$Isotope_Treatment<- paste(data.corr.melted.sed$Isotope_Treatment,"O",sep = "")
data.corr.melted.sed$Fraction<-as.factor(data.corr.melted.sed$Fraction)
data.corr.melted.sed$Tube<-as.factor(data.corr.melted.sed$Tube)
data.corr.melted.sed$Isotope_Treatment<-as.factor(data.corr.melted.sed$Isotope_Treatment)
data.corr.melted.sed$Soil_Isotope<-as.character(data.corr.melted.sed$Soil)
data.corr.melted.sed$Sample_ID<-as.character(data.corr.melted.sed$Sample_ID)
data.corr.melted.sed$taxon<-as.factor(data.corr.melted.sed$taxon)

test<- data.corr.melted.sed %>%
  filter(rel.abundance >0)
length(unique(test$taxon))
